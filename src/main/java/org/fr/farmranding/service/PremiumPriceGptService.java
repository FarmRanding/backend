...
public class PremiumPriceGptService {
    ...
    private String createPremiumPricePrompt(PremiumPriceRequest request) {
        ...
        return String.format("""
            ...
            ### 2단계: 지역 데이터 우선 사용
            - 지역 데이터 존재 여부: %s
            - 거래 위치(%s)와 일치하는 지역 데이터가 있으면 해당 값을 우선 사용
            - 해당 지역 데이터가 없으면 전국 평균값을 사용
            
            ### 3단계: 5일 평균 계산 (반드시 검산)
            - **정규화된 1개당 가격**으로 작년 동일한 시기 앞뒤 5일간의 소매/도매 각각 평균 가격을 계산
            - **중요**: 평균 계산을 두 번 검산하여 정확한 값 확인
            
            ### 4단계: 공식 적용 (단계별 계산)
            공식: 직거래 제안가 = min( 소매평균×0.95, max( 도매평균×1.20, 도매평균×α×0.5×0.9 ) )
            ...
            **4-2. 3개 값 계산:**
            - 값A = 소매 5일 평균 × 0.95
            - 값B = 도매 5일 평균 × 1.20  
            - 값C = 도매 5일 평균 × α × 0.5 × 0.9
            
            **4-3. 최종 계산:**
            - 먼저 max(값B, 값C) 계산
            - 그 다음 min(값A, max결과) 계산하여 최종 제안가 도출
            
            ### 5단계: 계산 검증
            - 모든 계산 과정을 한 번 더 검토
            - 소매평균, 도매평균, α값, 최종 제안가가 논리적으로 타당한지 확인
            
            ### 6단계: 설명 작성
            - calculationReason은 일반인이 이해하기 쉽고 신뢰성을 느낄 수 있도록 작성
            - kamis 공식 데이터를 사용했다는 문구는 이미 존재하므로 별도로 언급하지 마세요
            ...
            **중요**: 계산 과정을 내부적으로 꼼꼼히 검토한 후, 아래 JSON 형식으로만 응답해주세요.
            
            **계산 예시 (감자 데이터 기준):**
            - 소매 평균 = (2,763 + 2,792 + 2,791) ÷ 3 = 2,782원
            - 도매 평균 = (1,239 + 1,235 + 1,270) ÷ 3 = 1,248원  
            - α = 2,782 ÷ 1,248 = 2.23
            - 값A = 2,782 × 0.95 = 2,643원
            - 값B = 1,248 × 1.20 = 1,498원
            - 값C = 1,248 × 2.23 × 0.5 × 0.9 = 1,253원
            - max(값B, 값C) = max(1,498, 1,253) = 1,498원
            - 최종 = min(값A, 1,498) = min(2,643, 1,498) = 1,498원
            ...
            **필수 검증**: 모든 가격 값은 반드시 1개 기준으로 정규화된 값이어야 하며, 계산 결과가 논리적으로 타당해야 합니다.
            """,
            request.getItemName(),
            request.getKindName(),
            gradeDescription,
